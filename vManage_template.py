...
def login_information():
    ...
    username = input('Input your username:')
    password = input('Input your password:')
    print('Please cooy below export command to Cli')
    print("For Windows Workstation, vManage details must be set via environment variables using below commands")
    print('----------------------------------------------------------------------')
    print("set vmanage_host={}".format(vmanage_host))
    print("set vmanage_port={}".format(vmanage_port))
    ...
    print('----------------------------------------------------------------------')
    print("export vmanage_host={}".format(vmanage_host))
    print("export vmanage_port={}".format(vmanage_port))
    print("export username={}".format(username))
    print("export password={}".format(password))
    print('----------------------------------------------------------------------')
...
def list_devices():
    ...
    headers = ["Host-Name", "Device Type", "Latitude", "Longitude", "Certificate\nValidity", "Version", "Device Model",
               "System IP"]
    table = list()

    for item in items:
        if item["reachability"] == "reachable":
            tr = [item['host-name'], item['device-type'], item['latitude'], item['longitude'],
                  item['certificate-validity'], item['version'], item['device-model'], item['system-ip']]
            table.append(tr)
    try:
...
def control_status(system_ip):
    ...
    for item in items:
        tr = [item['peer-type'], item['system-ip'], item['state'],
              time.strftime('%m/%d/%Y %H:%M:%S', time.gmtime(item['lastupdated'] / 1000.))]
        table.append(tr)
...
def device_counters(system_ip):
    ...
    for item in items:
        try:
            tr = [item['ompPeersUp'], item['ompPeersDown'], item['number-vsmart-control-connections'],
                  item['bfdSessionsUp'], item['bfdSessionsDown']]
            table.append(tr)
        except KeyError:
            pass
...
def template_list():
    ...
    response_detail=vmanage_session.get_request('template/device/migration?hasAAA=false').json()

    items_detail=response_detail
    print(items_detail)
